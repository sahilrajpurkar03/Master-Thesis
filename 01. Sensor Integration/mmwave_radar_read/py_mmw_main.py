        ...
        while running:
            if not data_queue.empty():
                ...
                if dataOk:
                    ...
                    self.save_histogram(detObj.get("range", []), detObj.get("azimuth", []), currentIndex)
 
            #time.sleep(interval/1000)

    def save_histogram(self, range_data, azimuth_data, frame_number):
        """Create and save a range-azimuth 2D histogram for each frame."""
        output_dir = "/home/dartagnan-dev/sahil-dev/Sensor_integration/py_mmwave_dev/py_mmwave_read/range-azimuth-2D-Histogram"
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f"frame_{frame_number:04d}.jpg")
        
        self.create_heatmap(range_data, azimuth_data, output_path)
        print(f"Saved histogram for frame {frame_number} to {output_path}")
        

    def create_heatmap(self, range_data, azimuth_data, output_path, xlim=(-70, 70), ylim=(0, 6.5)):
